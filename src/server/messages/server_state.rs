...
mod tests {
  ...
  async fn test_handle_server_state_updates_peer_topics() {
    ...
    let server = Server::new(vec![peer_addr], crate::util::Address::from_str("127.0.0.1:0").unwrap());
    ...
    server
      .clone()
      .handle_server_state(ServerStateMessage {
        message_type: MessageType::ServerState as i32,
  ...
  #[tokio::test]
  async fn test_handle_server_state_unknown_peer_is_noop() {
    let server = Server::new(Vec::new(), crate::util::Address::from_str("127.0.0.1:0").unwrap());
    server
      .clone()
      .handle_server_state(ServerStateMessage {
        message_type: MessageType::ServerState as i32,
